...
function CornerMark({ color, position }: { color: string; position: 'tl' | 'br' }) {
	const edge = position === 'tl' ? { top: -13, left: -13 } : { bottom: -13, right: -13 }
	return (
		<div
			style={{
				position: 'absolute',
		...
		>
			...
			>
				...
				<div style={{ display: 'flex', flexDirection: 'column' }}>
					...
					{subtitle ? (
						<div style={{ fontSize: 28, color: palette.muted, marginTop: 28 }}>{subtitle}</div>
					) : null}
				</div>
			</div>
		</div>,
